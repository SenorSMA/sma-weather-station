...
/* The procedure "elevation_zenith_sun" computes the solar elevation (or
   ...
   MODIFIED: Removed clamping to 0 to allow negative values at night. */
{
 int ier;
 double phi;
 ...
 // Calculate gamma for all times, not just between sunrise/sunset
 *gamma = asin( sin(phi) * sin(delta) + cos(phi) * cos(delta) * cos(omega) );

 // Allow negative gamma values (sun below horizon)
 // Removed: if(*gamma < 0.0) *gamma = 0.0;
        ...
        if (Day_Angle (julian_day, &day_angle)==0)
        {
          ...
          {            
            ...
            {              
              ...
              {
                // MODIFIED: Removed gamma>0.0 check to show sun position 24/7
...
bool calcSunriseSunset (char *sunriseStr, char *sunsetStr, int bufSize)
{
  if (!Bolbro.timeConfigured()) return false;

  const double deg_rad = M_PI / 180.0;
  double phi_g  = deg2rad(LATITUDE);
  double lambda = deg2rad(LONGITUDE);
  double delta, day_angle, omega_sr, omega_ss;
  int year_number, month_number, day_of_month, julian_day;

  time_t t = time(NULL);
  struct tm *utcTime = gmtime(&t);

  year_number  = utcTime->tm_year + 1900;
  month_number = utcTime->tm_mon  + 1;
  day_of_month = utcTime->tm_mday;

  if (make_julian_day(day_of_month, month_number, year_number, &julian_day) != 0) return false;
  if (declination_sun(year_number, julian_day, lambda, &delta)              != 0) return false;
  if (Day_Angle(julian_day, &day_angle)                                     != 0) return false;
  if (sunrise_hour_angle(phi_g, delta, -1.0, &omega_sr, &omega_ss)          != 0) return false;

  // Convert hour angles to Local Apparent Time (decimal hours)
  double LAT_sr = omega_sr * 12.0 / M_PI + 12.0;
  double LAT_ss = omega_ss * 12.0 / M_PI + 12.0;

  // Equation of time (same coefficients as UT_to_LAT)
  double a1 = -0.128, a2 = -0.165;
  double a3 = 2.80 * deg_rad, a4 = 19.70 * deg_rad;
  double ET = a1 * sin(day_angle - a3) + a2 * sin(2.0 * day_angle + a4);

  // Convert LAT to UTC
  double UT_sr = LAT_sr - ET - (lambda * 12.0 / M_PI);
  double UT_ss = LAT_ss - ET - (lambda * 12.0 / M_PI);

  // Wrap to 0-24
  while (UT_sr <  0.0)  UT_sr += 24.0;
  while (UT_sr >= 24.0) UT_sr -= 24.0;
  while (UT_ss <  0.0)  UT_ss += 24.0;
  while (UT_ss >= 24.0) UT_ss -= 24.0;

  // Build time_t for today's midnight UTC, then add sunrise/sunset offsets
  time_t today_midnight = t - (time_t)(utcTime->tm_hour * 3600 + utcTime->tm_min * 60 + utcTime->tm_sec);
  time_t t_sr = today_midnight + (time_t)(UT_sr * 3600.0);
  time_t t_ss = today_midnight + (time_t)(UT_ss * 3600.0);

  // Format as local time — strftime copies into buffer before next call
  struct tm *local_sr = localtime(&t_sr);
  strftime(sunriseStr, bufSize, "%I:%M %p", local_sr);
  struct tm *local_ss = localtime(&t_ss);
  strftime(sunsetStr,  bufSize, "%I:%M %p", local_ss);

  // Strip leading zero from hour (e.g. "06:45 AM" -> "6:45 AM")
  if (sunriseStr[0] == '0') memmove(sunriseStr, sunriseStr + 1, strlen(sunriseStr));
  if (sunsetStr[0]  == '0') memmove(sunsetStr,  sunsetStr  + 1, strlen(sunsetStr));
